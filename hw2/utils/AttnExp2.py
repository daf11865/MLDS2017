...
class Model(object):
	...
	def get_config_str(self):
		return str("AttnExp2_Dr2_hs{}".format(self.hid_size))
	...
	def build_model_train(self, V_feat, C_idx, Embedding, Sd_rate):
		...
		with tf.variable_scope("C_LSTM") as scope:
			self.w_attn_match = tf.get_variable("w_attn_match", initializer=tf.truncated_normal_initializer(stddev=1e-4), shape=[self.hid_size,self.hid_size])
			self.w_out = tf.get_variable("w_out", initializer=tf.truncated_normal_initializer(stddev=1e-4), shape=[self.hid_size*2,ebd_size])
			#self.w_out = tf.get_variable("w_out", initializer=tf.truncated_normal_initializer(stddev=1e-4), shape=[self.hid_size,ebd_size-32])
			#self.w_out_ext = tf.get_variable("w_out_ext", initializer=tf.truncated_normal_initializer(stddev=1e-4), shape=[32,self.hid_size,self.hid_size])
			...
			for i in range(self.dec_step):
				...
				match = tf.reshape(tf.matmul(tf.reshape(tf.layers.dropout(v_enc_output, rate = 0.5, training = True),[-1,self.hid_size]), self.w_attn_match), [batch_size,-1,self.hid_size]) # [64,80,512]
				match = tf.matmul(match, tf.expand_dims(c_dec_output, axis = 2)) # [64,80,1]
				match = tf.nn.softmax(match) # [64,80,1]
		...
		for i in range(self.dec_step):
			if i == 0:
				with tf.variable_scope("C_LSTM",reuse = True):
					...
					match = tf.reshape(tf.matmul(tf.reshape(v_enc_output,[-1,self.hid_size]), self.w_attn_match), [1,-1,self.hid_size])
					match = tf.matmul(match, tf.expand_dims(c_dec_output, axis = 2))
					match = tf.nn.softmax(match)
					attn = tf.reduce_sum(tf.multiply(match, v_enc_output), axis = 1)
				...
				with tf.variable_scope("C_LSTM",reuse = True):
					...
					match = tf.reshape(tf.matmul(tf.reshape(v_enc_output,[-1,self.hid_size]), self.w_attn_match), [beam_width,-1,self.hid_size])
					match = tf.matmul(match, tf.expand_dims(c_dec_output, axis = 2))
					match = tf.nn.softmax(match)
					attn = tf.reduce_sum(tf.multiply(match, v_enc_output), axis = 1)
