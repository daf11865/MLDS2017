...
class Model(object):
	...
	def get_config_str(self):
		return str("FullyAttnP_Dr1_te{}_hs{}".format(self.tie_ebd, self.hid_size))
	...
	def build_model_train(self, V_feat, C_idx, Embedding, Sd_rate):
		...
		with tf.variable_scope("V_LSTM") as scope:
			...
			v_enc_output_1, v_enc_state_1 = tf.nn.dynamic_rnn(cell = self.v_cell, dtype = tf.float32, inputs = v_enc_ebd)
			scope.reuse_variables()
			v_enc_in_2 = tf.transpose(tf.gather(tf.transpose(v_enc_output_1, [1,0,2]), [2*t for t in range(20)]), [1,0,2])
			v_enc_output_2, v_enc_state_2 = tf.nn.dynamic_rnn(cell = self.v_cell, dtype = tf.float32, inputs = v_enc_in_2)
			v_enc_in_3 = tf.transpose(tf.gather(tf.transpose(v_enc_output_2, [1,0,2]), [2*t for t in range(10)]), [1,0,2])
			v_enc_output_3, v_enc_state_3 = tf.nn.dynamic_rnn(cell = self.v_cell, dtype = tf.float32, inputs = v_enc_in_3)

			v_enc_output = v_enc_output_3
		...
		with tf.variable_scope("C_LSTM") as scope:
			self.w_attn_c = tf.get_variable("w_attn_c", initializer=tf.truncated_normal_initializer(stddev=2/self.hid_size), shape=[self.hid_size,self.hid_size])
			self.w_attn_v = tf.get_variable("w_attn_v", initializer=tf.truncated_normal_initializer(stddev=2/self.hid_size), shape=[self.hid_size,self.hid_size])
			self.b_attn = tf.get_variable("b_attn", initializer=tf.truncated_normal_initializer(stddev=1e-4), shape=[1,1,self.hid_size])
			self.v_attn = tf.get_variable("v_attn", initializer=tf.truncated_normal_initializer(stddev=2/self.hid_size), shape=[self.hid_size,1])

			self.w_dattn_pre = tf.get_variable("w_dattn_pre",initializer=tf.truncated_normal_initializer(stddev=2/self.hid_size), shape=[self.hid_size,self.hid_size])
			self.w_dattn_cur = tf.get_variable("w_dattn_cur",initializer=tf.truncated_normal_initializer(stddev=2/self.hid_size), shape=[self.hid_size,self.hid_size])
			self.b_dattn = tf.get_variable("b_dattn", initializer=tf.truncated_normal_initializer(stddev=1e-4), shape=[1,1,self.hid_size])
			self.v_dattn = tf.get_variable("v_dattn", initializer=tf.truncated_normal_initializer(stddev=2/self.hid_size), shape=[self.hid_size,1])
			...
			c_dec_idx = C_idx[:,0]
			c_dec_output_collection = tf.zeros([batch_size,0,self.hid_size])
			for i in range(self.dec_step):
				...
				c_dec_output_collection = tf.concat([c_dec_output_collection, tf.expand_dims(c_dec_output,axis=1)], axis=1) # [64,i,512]
				dattn_pre = tf.reshape(tf.matmul(tf.reshape(c_dec_output_collection,[-1,self.hid_size]), self.w_dattn_pre), [batch_size,-1,self.hid_size]) # [64,i,512]
				dattn_cur = tf.reshape(tf.matmul(c_dec_output, self.w_dattn_cur), [batch_size,1,self.hid_size]) # [64,1,512]
				dattn_pre_cur_b = dattn_pre + dattn_cur + self.b_dattn # [64,i,512]
				dmatch = tf.reshape(tf.matmul(tf.reshape(dattn_pre_cur_b, [-1,self.hid_size]), self.v_dattn), [batch_size,-1]) # [64,i]
				dmatch = tf.expand_dims(tf.nn.softmax(dmatch), axis = 2) # [64,i,1]
				dattn = tf.reduce_sum(tf.multiply(dmatch, c_dec_output_collection), axis = 1) # [64,512]

				concat = tf.concat([dattn, attn],axis = 1) # [64,1024]
	...
	def build_model_inference(self, V_feat, Embedding, beam_width = 5):
		...
		c_dec_state = (c_zero_state[0], tf.concat([c_zero_state[1]]*2, axis = 1))
		c_dec_output_collection = tf.zeros([bw,0,self.hid_size])
		for i in range(self.dec_step):
			with tf.variable_scope("C_LSTM",reuse = True):
				...
				attn_v = tf.reshape(tf.matmul(tf.reshape(v_enc_output,[-1,self.hid_size]), self.w_attn_v), [bw,-1,self.hid_size]) # [64,80,512]
				attn_c = tf.reshape(tf.matmul(c_dec_output, self.w_attn_c), [bw,1,self.hid_size]) # [64,1,512]
				attn_v_c_b = attn_v + attn_c + self.b_attn # [64,80,512]
				match = tf.reshape(tf.matmul(tf.reshape(attn_v_c_b, [-1,self.hid_size]), self.v_attn), [bw,-1]) # [64,80]
				match = tf.expand_dims(tf.nn.softmax(match), axis = 2) # [64,80,1]
				attn = tf.reduce_sum(tf.multiply(match, v_enc_output), axis = 1) # [64,512]

				c_dec_output_collection = tf.concat([c_dec_output_collection, tf.expand_dims(c_dec_output,axis=1)], axis=1) # [64,i,512]
				dattn_pre = tf.reshape(tf.matmul(tf.reshape(c_dec_output_collection,[-1,self.hid_size]), self.w_dattn_pre), [bw,-1,self.hid_size]) # [64,i,512]
				dattn_cur = tf.reshape(tf.matmul(c_dec_output, self.w_dattn_cur), [bw,1,self.hid_size]) # [64,1,512]
				dattn_pre_cur_b = dattn_pre + dattn_cur + self.b_dattn # [64,i,512]
				dmatch = tf.reshape(tf.matmul(tf.reshape(dattn_pre_cur_b, [-1,self.hid_size]), self.v_dattn), [bw,-1]) # [64,i]
				dmatch = tf.expand_dims(tf.nn.softmax(dmatch), axis = 2) # [64,i,1]
				dattn = tf.reduce_sum(tf.multiply(dmatch, c_dec_output_collection), axis = 1) # [64,512]

				concat = tf.concat([dattn, attn],axis = 1)
				c_dec_state = (c_dec_state[0], concat)
				out_ebd = tf.matmul(concat, self.w_out)
			...
			if i == 0:
				...
				c_dec_output_collection = tf.concat([c_dec_output_collection]*beam_width,axis = 0)
				bw = beam_width
