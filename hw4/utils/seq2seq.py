...
# ==============================================================================
"""Attention-based monte carlo decoder functions."""
...
__all__ = [
    "attention_monte_carlo_decoder_fn_inference"
]


def attn_monte_carlo_dec_fn_inference(output_fn,
                                   encoder_state,
                                   attention_keys,
                                   attention_values,
                                   attention_score_fn,
  ...
  def decoder_fn(time, cell_state, cell_input, cell_output, context_state):
    ...
    with ops.name_scope(
      ...
      if cell_output is None:
        ...
        # argmax decoder
        cell_output = nn_ops.softmax(output_fn(cell_output))  # XXX: probs
        next_input_id = math_ops.cast(array_ops.reshape(random_ops.multinomial(math_ops.log(cell_output), 1), [-1]), dtype=dtype) # XXX: monte carlo instead of argmax
        done = math_ops.equal(next_input_id, end_of_sequence_id)
        cell_input = array_ops.gather(embeddings, next_input_id)
        emit_output = math_ops.multiply(array_ops.one_hot(next_input_id, num_decoder_symbols, 1., 0.), cell_output) # XXX: one_hot probs
      ...
      # if time > maxlen, return all true vector
          ...
          lambda: done)
      return (done, cell_state, next_input, emit_output, context_state) # XXX:
...
def simple_monte_carlo_dec_fn_inference(input_fn, output_fn, encoder_state, embeddings,
  ...
  """
    with ops.name_scope(name, "simple_decoder_fn_inference",
      ...
      next_input = input_fn(array_ops.gather(embeddings, next_input_id))
      # if time > maxlen, return all true vector
...
def simple_dec_fn_inference(input_fn, output_fn, encoder_state, embeddings,
                                start_of_sequence_id, end_of_sequence_id,
                                maximum_length, num_decoder_symbols,
                                dtype=dtypes.int32, name=None):
  """ Simple decoder function for a sequence-to-sequence model used in the
