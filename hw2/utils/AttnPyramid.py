...
Dr2 : v_enc_ebd, v_enc_out(excluding hidden state)(should this one not dropout due to not syncornize v_enc_out?)
"""
...
class Model(object):
	...
	def get_config_str(self):
		return str("AttnP_Dr1_te{}_hs{}".format(self.tie_ebd, self.hid_size))
	...
	def build_model_train(self, V_feat, C_idx, Embedding, Sd_rate):
		...
		with tf.variable_scope("V_LSTM") as scope:
			...
			v_enc_in_2 = tf.transpose(tf.gather(tf.transpose(v_enc_output_1, [1,0,2]), [2*t+1 for t in range(40)]), [1,0,2])
			v_enc_output_2, v_enc_state_2 = tf.nn.dynamic_rnn(cell = self.v_cell, dtype = tf.float32, inputs = v_enc_in_2)
			v_enc_in_3 = tf.transpose(tf.gather(tf.transpose(v_enc_output_2, [1,0,2]), [2*t+1 for t in range(20)]), [1,0,2])
